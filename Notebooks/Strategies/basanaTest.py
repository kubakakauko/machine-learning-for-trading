...
SYMBOL = "BTCUSDT"
TIMEFRAME = "5s"  # Fetch 1-minute candles
...
class BTCLiveChart(QMainWindow):
    ...
    def process_websocket_message(self, trade):
        ...
        timestamp = int(timestamp // (AGG_INTERVAL_MINUTES * 60)) * (
        ...
        if timestamp not in self.data.index:
            ...
            self.data.at[timestamp, "Close"] = price
            self.data.at[timestamp, "High"] = max(
                self.data.at[timestamp, "High"], price
            )
            self.data.at[timestamp, "Low"] = min(self.data.at[timestamp, "Low"], price)
            self.data.at[timestamp, "Volume"] += volume
    ...
    def plot_trades(self):
        ...
        if hasattr(self, "trade_scatter"):
            self.graph_widget.removeItem(self.trade_scatter)
        ...
        for trade in self.trades:
            ...
            spots.append(
                {
                    "pos": (entry_timestamp, trade.EntryPrice),
            ...
            if trade.ExitTime and trade.ExitPrice:
                ...
                spots.append(
                    {
                        "pos": (exit_timestamp, trade.ExitPrice),
        ...
        # Current trade entry point
        if self.current_trade:
            entry_timestamp = int(self.current_trade.EntryTime.timestamp())
            spots.append(
                {
                    "pos": (entry_timestamp, self.current_trade.EntryPrice),
        ...
        # Create ScatterPlotItem and add to the graph
        self.trade_scatter = pg.ScatterPlotItem(size=10, pen=pg.mkPen(None))
        self.trade_scatter.addPoints(spots)
        self.graph_widget.addItem(self.trade_scatter)
